...
AniMan — a Windows-only WPF desktop app (net10.0-windows) for tracking anime and manga, backed by the public Jikan (MyAnimeList) API. UI is WPF UI 3 (Fluent), MVVM via CommunityToolkit source generators, persistence via EF Core 9 + SQLite.
...
- **AniMan.Infrastructure** — EF contexts, the Jikan client, and all service implementations. Registered in one place: `ServiceCollectionExtensions.AddInfrastructure(appDataPath)`.
...
- `catalog.db` — cache of Jikan responses (`CachedAnime`, `CachedEpisode`, `CachedGenre`, …). Disposable; deleting it only costs a re-fetch.
