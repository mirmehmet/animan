    ...
    ITrackingService trackingService) : ObservableObject
{
    ...
    private void ExportData()
    {
        var dlg = new SaveFileDialog
        {
            ...
            Filter = "AniTrack Backup (*.anitrack)|*.anitrack|SQLite DB (*.db)|*.db",
            FileName = $"anitrack-backup-{DateTime.Today:yyyy-MM-dd}"
        };
    ...
    private void ImportData()
    {
        var dlg = new OpenFileDialog
        {
            ...
            Filter = "AniTrack Backup (*.anitrack;*.db)|*.anitrack;*.db"
        };
        if (dlg.ShowDialog() != true) return;
